...
use crate::{
    ...
    corpus::{testcase::RLFuzzTestcaseMetaData, Corpus, SchedulerTestcaseMetaData, Testcase},
...
#[derive(Debug, Clone)]
pub struct SeedSelectionTestcaseScore<S> {
    phantom: PhantomData<S>,
}

impl<S> TestcaseScore<S> for SeedSelectionTestcaseScore<S>
where
    S: HasCorpus + HasMetadata,
{
    fn compute(entry: &mut Testcase<S::Input>, _state: &S) -> Result<f64, Error> {
        Ok(*entry.executions() as f64)
    }
}

/// Time spent on fuzzing seed so far
#[derive(Debug, Clone)]
pub struct SeedTimeTestcaseScore<S> {
    phantom: PhantomData<S>,
}

impl<S> TestcaseScore<S> for SeedTimeTestcaseScore<S>
where
    S: HasCorpus + HasMetadata,
{
    fn compute(entry: &mut Testcase<S::Input>, _state: &S) -> Result<f64, Error> {
...
#[derive(Debug, Clone)]
pub struct GeneratedTestcaseScore<S> {
    phantom: PhantomData<S>,
}

impl<S> TestcaseScore<S> for GeneratedTestcaseScore<S>
where
    S: HasCorpus + HasMetadata,
{
    fn compute(entry: &mut Testcase<S::Input>, _state: &S) -> Result<f64, Error> {
